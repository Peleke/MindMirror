...
class GCSClient:
    ...
    def __init__(self):
        ...
        if self.use_emulator:
            ...
            logger.info(f"Using local GCS emulator at: {self.local_bucket_path.resolve()}")
        else:
            if not self.bucket_name:
                raise ValueError("GCS_BUCKET_NAME must be set when not using the emulator.")
    ...
    def download_to_filename(self, blob_name: str, destination_filename: str):
        ...
        if self.use_emulator:
            ...
            with open(source_path, 'rb') as f_in, open(destination_filename, 'wb') as f_out:
