...
def trigger_reindex_on_gcs_upload(cloud_event):
    ...
    if not all([api_webhook_url, reindex_secret]):
        logging.error(
            "Missing required environment variables: API_WEBHOOK_URL or REINDEX_SECRET_KEY."
        )
    ...
    try:
        ...
        logging.info(
            f"Successfully triggered webhook. Status: {response.status_code}, Response: {response.text}"
        )
