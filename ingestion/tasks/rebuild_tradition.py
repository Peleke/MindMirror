...
def rebuild_tradition_knowledge_base(tradition: str):
    ...
    try:
        ...
        logger.warning(
            f"Could not delete collection {knowledge_collection_name} (it may not exist): {e}"
        )
    ...
    if not doc_blobs:
        logger.warning(
            f"No documents found in GCS for tradition '{tradition}' with prefix '{doc_prefix}'."
        )
    ...
    for blob in doc_blobs:
        ...
        try:
            ...
            ]  # This should be batched in a real scenario
            ...
            logger.info(
                f"Successfully processed and indexed {len(docs)} chunks from {blob.name}"
            )
