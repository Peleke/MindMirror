...
from src.vector_stores.qdrant_client import QdrantClient, SearchResult
...
class TestQdrantClient:
    ...
    async def test_knowledge_collection_creation(self, qdrant_client: QdrantClient):
        ...
        collection_name = await qdrant_client.get_or_create_knowledge_collection(tradition)
        
        assert collection_name == tradition
    ...
    async def test_knowledge_document_indexing(self, qdrant_client: QdrantClient):
        ...
        point_id = await qdrant_client.index_knowledge_document(
            tradition=tradition,
            text=text,
            embedding=embedding,
            metadata=metadata
    ...
    async def test_personal_document_indexing(self, qdrant_client: QdrantClient):
        ...
        point_id = await qdrant_client.index_personal_document(
            tradition=tradition,
            user_id=user_id,
            text=text,
            embedding=embedding,
    ...
    async def test_metadata_filtering(self, qdrant_client: QdrantClient):
        ...
        gratitude_results = await qdrant_client.search_documents(
            collection_name=collection_name,
            query_embedding=embedding,
            limit=10,
        ...
        reflection_results = await qdrant_client.search_documents(
            collection_name=collection_name,
            query_embedding=embedding,
            limit=10,
    ...
    async def test_user_isolation(self, qdrant_client: QdrantClient):
        ...
        collection1 = await qdrant_client.get_or_create_personal_collection(tradition, user1_id)
        collection2 = await qdrant_client.get_or_create_personal_collection(tradition, user2_id)
    ...
    async def test_hybrid_search(self, qdrant_client: QdrantClient):
        ...
        await qdrant_client.index_knowledge_document(
            tradition=tradition,
        ...
        knowledge_only = await qdrant_client.hybrid_search(
            query="stoic resilience practices",
            user_id=user_id,
            tradition=tradition,
            query_embedding=embedding,
        ...
        personal_only = await qdrant_client.hybrid_search(
            query="stoic resilience practices",
            user_id=user_id,
            tradition=tradition,
            query_embedding=embedding,
    ...
    async def test_collection_cleanup(self, qdrant_client: QdrantClient):
        ...
        info_after = await qdrant_client.get_collection_info(collection_name)
