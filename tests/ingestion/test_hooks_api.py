...
async def test_trigger_reindex_success(client):
    ...
    with patch.dict("os.environ", {"REINDEX_SECRET_KEY": secret}):
        with patch("src.web.hooks.celery_app.send_task") as mock_send_task:
            ...
            assert response.json() == {"message": "Accepted re-indexing task for tradition: canon-default"}
...
async def test_trigger_reindex_task_queue_fails(client):
    ...
    secret = "test-secret"
    with patch.dict("os.environ", {"REINDEX_SECRET_KEY": secret}):
        with patch("src.web.hooks.celery_app.send_task", side_effect=Exception("Queueing failed")):
            response = await client.post(
                "/triggers/reindex-tradition",
                params={"tradition": "canon-default"},
                headers={"X-Reindex-Secret": secret}
