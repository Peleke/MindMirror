...
class FAISSToQdrantMigrator:
    ...
    async def migrate_tradition(self, tradition: str, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            documents, embeddings = await self._extract_documents_from_faiss(vector_store)
    ...
    async def _extract_documents_from_faiss(self, vector_store) -> tuple[List[Dict], List[List[float]]]:
    ...
    async def _migrate_documents_to_qdrant(
        ...
        knowledge_collection = await self.qdrant_client.get_or_create_knowledge_collection(tradition)
        
        logger.info(f"Migrating {len(documents)} documents to knowledge collection: {knowledge_collection}")
            ...
            for doc, embedding in zip(batch_docs, batch_embeddings):
                try:
                    ...
                    if stats["knowledge_documents"] % 100 == 0:
                        logger.info(f"Migrated {stats['knowledge_documents']} documents...")
    ...
    async def verify_migration(self, tradition: str, user_id: str, sample_query: str = "strength training") -> bool:
        ...
        try:
            ...
            logger.info(f"Found {len(knowledge_results)} results in knowledge collection")
    ...
    for tradition in traditions:
        ...
        try:
            ...
            if not dry_run and "error" not in stats:
                verification_success = await migrator.verify_migration(tradition, user_id)
...
def main():
    """Main CLI function."""
    parser = argparse.ArgumentParser(description="Migrate FAISS vector stores to Qdrant")
    parser.add_argument(
        "--tradition", 
        type=str, 
    ...
    parser.add_argument(
        "--all-traditions", 
        action="store_true",
        help="Migrate all available traditions"
    )
    parser.add_argument(
        "--dry-run", 
        action="store_true",
    ...
    async def run_migration():
        try:
            if args.verify_only:
                ...
                if args.tradition:
                    success = await migrator.verify_migration(args.tradition, args.user_id)
            ...
            for tradition, stats in results.items():
                ...
                if "error" in stats:
                    ...
                    if "verification_passed" in stats:
                        status = "✅ PASSED" if stats["verification_passed"] else "❌ FAILED"
