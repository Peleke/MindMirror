    ...
    <Pressable className="w-full" onPress={onOpen}>
      <Box className="p-4 rounded-lg border bg-indigo-50 dark:bg-indigo-950 border-indigo-200 dark:border-indigo-800 shadow">
        <VStack space="sm">
          ...
          {task.summary ? (
            <Text className="text-typography-600 dark:text-gray-300">{task.summary}</Text>
          ) : null}
          <Text onPress={onComplete} className="text-primary-600 dark:text-primary-400">Mark completed</Text>
        </VStack>
      </Box>
    </Pressable>
  )
