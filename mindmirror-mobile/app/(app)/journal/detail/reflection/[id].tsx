...
export default function ReflectionDetailScreen() {
  ...
  const entry = data.journalEntry as ReflectionJournalEntry;

  return (
    <SafeAreaView className="h-full w-full">
      <VStack className="h-full w-full bg-background-0">
        <AppBar title="Reflection Entry" showBackButton onBackPress={handleBackPress} />
          ...
          <VStack className="px-6 pb-6" space="md">
            <Box className="p-6 bg-indigo-50 dark:bg-indigo-950 rounded-lg border border-border-200 dark:border-border-700">
              <VStack space="md">
                <ReadOnlyField
                  label="Today's Wins"
                  value={entry.payload.wins.join('\n')}
                  numberOfLines={4}
                />
                
                <ReadOnlyField
                  label="Areas for Improvement"
                  value={entry.payload.improvements.join('\n')}
                  numberOfLines={4}
                />
