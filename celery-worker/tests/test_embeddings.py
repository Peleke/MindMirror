...
class TestOllamaEmbeddingService:
    ...
    async def test_get_embeddings_multiple_texts(self):
        ...
        mock_response = Mock()
        mock_response.raise_for_status.return_value = None
        mock_response.json.return_value = {"embedding": [0.1] * Config.VECTOR_SIZE}
...
class TestSentenceTransformersEmbeddingService:
    ...
    def test_init_default_model(self, mock_import):
        ...
        mock_import.side_effect = lambda name, *args: (
            Mock() if name == "sentence_transformers" else __import__(name, *args)
        )
