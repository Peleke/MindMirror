...
from ..repository.models import MovementModel
...
async def import_pn_movements_csv(
    ...
    - Jen (female coach) -> long_video_url
    - Craig (male coach) -> short_video_url
    ...
    for exercise_name in sorted(all_exercise_names):
        ...
        stmt = select(MovementModel).where(MovementModel.name == exercise_name)
        ...
        try:
            if existing:
                ...
                # Create new movement
                await repo.create(user_id=None, data=movement_data)
                created += 1
