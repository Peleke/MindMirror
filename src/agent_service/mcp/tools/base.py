...
class ToolRegistry:
    ...
    def get_tool_metadata(self, name: str, version: Optional[str] = None) -> Optional[ToolMetadata]:
        ...
        if version:
            ...
            try:
                versions = sorted(self._metadata[name].keys(), key=parse_version, reverse=True)
    ...
    def export_to_yaml(self) -> str:
        ...
        for metadata in self.list_tools():
            ...
            export_data["metadata"]["backends"][backend] = export_data["metadata"]["backends"].get(backend, 0) + 1
            export_data["metadata"]["domains"][domain] = export_data["metadata"]["domains"].get(domain, 0) + 1
            export_data["metadata"]["effect_boundaries"][effect_boundary] = export_data["metadata"]["effect_boundaries"].get(effect_boundary, 0) + 1
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        for metadata in self.list_tools():
            ...
            effect_boundaries[effect_boundary] = effect_boundaries.get(effect_boundary, 0) + 1
