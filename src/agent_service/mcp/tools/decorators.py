...
def register_tool(
    ...
    def decorator(cls: T) -> T:
        ...
        try:
            ...
            raise ValueError(
                f"Invalid backend: {backend}. Must be one of {[b.value for b in ToolBackend]}"
            )
        ...
        try:
            ...
            raise ValueError(
                f"Invalid effect_boundary: {effect_boundary}. Must be one of {[e.value for e in EffectBoundary]}"
            )
        ...
        final_output_schema = (
            output_schema if output_schema is not None else default_output_schema
        )
        ...
        if not hasattr(cls, "input_schema") or cls.input_schema.__get__(
            None, cls
        ) == MCPTool.input_schema.__get__(None, MCPTool):
            cls.input_schema = input_schema_property

        if not hasattr(cls, "output_schema") or cls.output_schema.__get__(
            None, cls
        ) == MCPTool.output_schema.__get__(None, MCPTool):
            cls.output_schema = output_schema_property

        if not hasattr(cls, "owner_domain") or cls.owner_domain.__get__(
            None, cls
        ) == MCPTool.owner_domain.__get__(None, MCPTool):
            cls.owner_domain = owner_domain_property

        if not hasattr(cls, "version") or cls.version.__get__(
            None, cls
        ) == MCPTool.version.__get__(None, MCPTool):
            cls.version = version_property

        # Add subtools support if not already implemented
        if not hasattr(cls, "list_subtools") or cls.list_subtools.__get__(
            None, cls
        ) == MCPTool.list_subtools.__get__(None, MCPTool):
