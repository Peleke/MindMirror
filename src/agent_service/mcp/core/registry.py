...
class MCPPluginRegistry:
        ...
        for plugin_path in plugin_dir.glob("*/__init__.py"):
            ...
            try:
                ...
                if plugin_class is None:
                    self.logger.warning(f"No plugin class found in {plugin_name}: {plugin_class_name}")
                    continue
    ...
    def create_plugin_instance(
        ...
        missing_deps = [dep for dep in plugin_info.dependencies if dep not in dependencies]
        if missing_deps:
    ...
    def filter_plugins_by_tags(self, tags: List[str]) -> List[PluginInfo]:
        ...
        """
        return [plugin for plugin in self._plugins.values() if any(tag in plugin.tags for tag in tags)]
