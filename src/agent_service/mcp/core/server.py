...
try:
    ...
    from mcp.types import ErrorData, Prompt, Resource, TextContent, Tool
...
from .base import MCPPlugin, MCPPrompt, MCPResource, MCPTool, create_checkpoint
        ...
        async def list_resources() -> List[Resource]:
            ...
            for plugin_name, plugin in self.plugins.items():
                try:
                    ...
                    self.logger.error(
                        f"Failed to get resources from plugin {plugin_name}: {e}"
                    )
        ...
        async def list_prompts() -> List[Prompt]:
            ...
            for plugin_name, plugin in self.plugins.items():
                try:
                    ...
                    self.logger.error(
                        f"Failed to get prompts from plugin {plugin_name}: {e}"
                    )
            ...
            try:
                ...
                for p_name, plugin in self.plugins.items():
                    try:
                        ...
                        self.logger.error(
                            f"Failed to get tools from plugin {p_name}: {e}"
                        )
                        continue
                ...
                if plugin_name is None:
                    raise McpError(
                        ErrorData(
                            code="INVALID_PARAMS", message=f"Unknown tool: {name}"
                        )
                    )
                ...
                for item in result:
                    if isinstance(item, dict):
                        if "type" in item and "text" in item:
                            content.append(
                                TextContent(type=item["type"], text=item["text"])
                            )
                        else:
                ...
                self.logger.info(
                    f"Executed tool {name} from plugin {plugin_name} in {execution_time:.3f}s"
                )
                return content
            ...
            try:
                ...
                for p_name, plugin in self.plugins.items():
                    try:
                        ...
                        self.logger.error(
                            f"Failed to get resources from plugin {p_name}: {e}"
                        )
                        continue
                ...
                if plugin_name is None:
                    raise McpError(
                        ErrorData(
                            code="INVALID_PARAMS", message=f"Unknown resource: {uri}"
                        )
                    )
                ...
                self.logger.info(
                    f"Read resource {uri} from plugin {plugin_name} in {execution_time:.3f}s"
                )
                return content
                ...
                raise McpError(
                    ErrorData(
                        code="INTERNAL_ERROR", message=f"Resource read failed: {str(e)}"
                    )
                )
            ...
            try:
                ...
                for p_name, plugin in self.plugins.items():
                    try:
                        ...
                        self.logger.error(
                            f"Failed to get prompts from plugin {p_name}: {e}"
                        )
                        continue
                ...
                if plugin_name is None:
                    raise McpError(
                        ErrorData(
                            code="INVALID_PARAMS", message=f"Unknown prompt: {name}"
                        )
                    )
                ...
                self.logger.info(
                    f"Got prompt {name} from plugin {plugin_name} in {execution_time:.3f}s"
                )
                return result
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        for plugin_name, plugin in self.plugins.items():
            try:
                ...
                plugin_health[plugin_name] = {"status": "error", "error": str(e)}
