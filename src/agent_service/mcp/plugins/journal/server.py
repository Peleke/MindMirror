...
class JournalPlugin(MCPPlugin):
    ...
    def _validate_dependencies(self) -> None:
        ...
        if not isinstance(self.dependencies["retriever_registry"], RetrieverRegistry):
            raise ValueError("retriever_registry must be an instance of RetrieverRegistry")
    ...
    def _create_tools(self) -> List[MCPTool]:
        ...
        for tool_name, metadata in tool_metadata.items():
            tool = MCPTool(
                ...
                tags=metadata["tags"],
                metadata={"category": metadata["category"], "uses_retriever": metadata["uses_retriever"]}
            )
            tools.append(tool)
    ...
    async def get_prompt_content(self, name: str, arguments: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    async def _summarize_journals(self, arguments: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    async def _get_entries_by_type(self, arguments: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if hasattr(retriever, 'get_entries_by_type'):
            ...
            documents = [
                doc for doc in documents 
                if doc.metadata.get("entry_type") == entry_type
            ]
    ...
    async def _get_summarize_prompt(self, arguments: Optional[Dict[str, Any]]) -> Dict[str, Any]:
