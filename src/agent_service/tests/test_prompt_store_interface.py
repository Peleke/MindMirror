...
class TestPromptStoreIntegration:
    ...
    def test_store_multiple_versions(self):
        ...
        store.get_prompt.side_effect = lambda name, version: (
            v1_info if version == "1.0" else v2_info
        )
...
class TestPromptStoreValidation:
    ...
    def test_store_handles_invalid_versions(self):
        ...
        store.delete_prompt.side_effect = lambda name, version: validate_version(
            version
        )
