...
class TestInMemoryPromptStore:
    ...
    def test_save_prompt(self):
        ...
        prompt = PromptInfo(
            name="test_prompt", version="1.0", content="Hello {{name}}!"
        )
    ...
    def test_get_stats(self):
        ...
        store.save_prompt(
            PromptInfo(name="prompt1", version="2.0", content="Content 2")
        )
        store.save_prompt(
            PromptInfo(name="prompt2", version="1.0", content="Content 3")
        )
...
class TestGCSPromptStore:
    ...
    def test_save_prompt(self):
        ...
        assert (
            "test_prompt/1.0.yaml" in call_args[0][0]
        )  # Path contains prompt name and version
    ...
    def test_list_prompts(self):
        ...
        # Mock file content
        prompt_data = {"name": "prompt1", "version": "1.0", "content": "Content 1"}
        mock_loader.read_file.return_value = yaml.dump(prompt_data)
        ...
        assert len(prompt_list) == 3
        assert all(
            p.name in ["prompt1", "prompt1", "prompt1"] for p in prompt_list
        )  # All mocked as prompt1
    ...
    def test_search_prompts(self):
        ...
        def mock_read_file(path):
            if "greeting" in path:
                return yaml.dump(
                    {"name": "greeting", "version": "1.0", "content": "Hello {{name}}!"}
                )
            else:
                return yaml.dump(
                    {"name": "other", "version": "1.0", "content": "Other content"}
                )
