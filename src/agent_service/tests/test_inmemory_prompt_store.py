...
class TestInMemoryPromptStore:
    ...
    def test_save_prompt(self, store, sample_prompt_info):
        ...
        assert (
            store._prompts[sample_prompt_info.name][sample_prompt_info.version]
            == sample_prompt_info
        )

        # Check that latest version is updated
        assert (
            store._latest_versions[sample_prompt_info.name]
            == sample_prompt_info.version
        )
    ...
    def test_get_prompt_version_not_found(self, store, sample_prompt_info):
        ...
        with pytest.raises(
            PromptNotFoundError,
            match="Version '2.0' not found for prompt 'test_prompt'",
        ):
    ...
    def test_get_latest_version_not_found(self, store):
        ...
        with pytest.raises(
            PromptNotFoundError, match="No versions found for prompt 'nonexistent'"
        ):
