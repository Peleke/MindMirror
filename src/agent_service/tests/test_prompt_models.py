...
class TestPromptInfo:
    ...
    def test_prompt_info_with_minimal_fields(self):
        ...
        prompt_info = PromptInfo(
            name="minimal_prompt", version="1.0", content="Simple content"
        )
    ...
    def test_prompt_info_validation(self):
        ...
        # Test empty name
        with pytest.raises(
            PromptValidationError, match="Prompt name must be a non-empty string"
        ):
            PromptInfo(name="", version="1.0", content="test")

        # Test empty version
        with pytest.raises(
            PromptValidationError, match="Version must be a non-empty string"
        ):
            PromptInfo(name="test", version="", content="test")

        # Test empty content
        with pytest.raises(
            PromptValidationError, match="Content must be a non-empty string"
        ):
...
class TestPromptConfig:
    ...
    def test_prompt_config_validation(self):
        ...
        with pytest.raises(
            PromptValidationError, match="store_type must be a StoreType enum"
        ):
            PromptConfig(store_type="invalid")

        # Test local storage without path
        with pytest.raises(
            PromptValidationError, match="store_path is required for LOCAL store type"
        ):
            PromptConfig(store_type=StoreType.LOCAL)

        # Test GCS storage without bucket
        with pytest.raises(
            PromptValidationError, match="gcs_bucket is required for GCS store type"
        ):
...
class TestStorageConfig:
    ...
    def test_valid_local_storage_config(self):
        ...
        config = StorageConfig(storage_type="local", local_path="/app/storage/prompts")
    ...
    def test_storage_config_validation_local(self):
        ...
        with pytest.raises(
            ValueError, match="local_path is required for local storage"
        ):
    ...
    def test_storage_config_validation_gcs(self):
        ...
        with pytest.raises(
            ValueError, match="gcs_credentials is required for GCS storage"
        ):
            StorageConfig(
                storage_type="gcs", gcs_bucket="test-bucket", gcs_credentials=""
            )
    ...
    def test_storage_config_serialization(self):
        ...
        config = StorageConfig(storage_type="local", local_path="/test/path")
    ...
    def test_storage_config_from_environment(self):
        ...
        config = StorageConfig(storage_type="local", local_path="/env/path")
    ...
    def test_storage_config_equality(self):
        ...
        config1 = StorageConfig(storage_type="local", local_path="/test/path")

        config2 = StorageConfig(storage_type="local", local_path="/test/path")
