...
class TestLangGraphNodeConfig:
    ...
    def test_node_config_defaults(self):
        ...
        config = LangGraphNodeConfig(
            node_type=LangGraphNodeType.LLM, name="llm_node", description="An LLM node"
        )
...
class TestLangGraphTool:
    ...
    def test_effect_boundary_mapping(self):
        ...
        llm_config = LangGraphNodeConfig(
            node_type=LangGraphNodeType.LLM, name="llm", description="An LLM"
        )
        ...
        tool_config = LangGraphNodeConfig(
            node_type=LangGraphNodeType.TOOL, name="tool", description="A tool"
        )
...
class TestLangGraphWorkflowTool:
    ...
    async def test_workflow_execution_with_conditionals(self):
            ...
            "nodes": [
                {
                    ...
                    "config": {
                        "condition": "has_data",
                        "branches": {"true": "process", "false": "end"},
                    },
                },
                {
                    "name": "process",
        ...
        result_with_data = await tool.execute(
            {"initial_state": {"has_data": True}, "workflow_params": {"max_steps": 5}}
        )
        ...
        result_without_data = await tool.execute(
            {"initial_state": {"has_data": False}, "workflow_params": {"max_steps": 5}}
        )
    ...
    def test_get_next_node(self):
        ...
        workflow_config = {
            ...
            "entry_point": "start",
            "nodes": [],
            "edges": [{"from": "start", "to": "next"}, {"from": "next", "to": "end"}],
        }
    ...
    def test_evaluate_condition(self):
        ...
        assert (
            tool._evaluate_condition("has_data", {"data": "value"}) is True
        )  # "data" exists and is truthy
        assert (
            tool._evaluate_condition("has_data", {"data": True}) is True
        )  # "data" exists and is truthy
        assert (
            tool._evaluate_condition("has_data", {"data": False}) is False
        )  # "data" exists but is falsy
