...
class TestPromptTool:
    ...
    async def test_execute_chain_prompt(self):
        ...
        prompt_config = PromptConfig(
            prompt_type=PromptType.CHAIN,
            content=json.dumps(chain_config)
        )
    ...
    async def test_execute_conditional_prompt(self):
        ...
        prompt_config = PromptConfig(
            prompt_type=PromptType.CONDITIONAL,
            content=json.dumps(conditional_config)
        )
    ...
    def test_evaluate_condition(self):
        ...
        # Test in condition
        assert tool._evaluate_condition("item in items", {"item": "apple", "items": ["apple", "banana"]}) is True
        assert tool._evaluate_condition("item in items", {"item": "orange", "items": ["apple", "banana"]}) is False
...
class TestPromptTemplateManager:
    ...
    def test_export_templates(self):
        ...
        assert exported["greeting"]["variables"] == {"name": {"type": "string", "required": True}}
