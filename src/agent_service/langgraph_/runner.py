...
class GraphRunner(Generic[StateT]):
    ...
    def run_stream(self, state: StateT):
        ...
        try:
            self.logger.info(
                f"Starting streaming graph execution for user {state['user_id']}"
            )
            ...
            self.logger.info(
                f"Streaming graph execution completed for user {state['user_id']}"
            )
    ...
    def validate_state(self, state: StateT) -> bool:
        ...
        has_tradition = ("tradition" in state and state["tradition"] is not None) or (
