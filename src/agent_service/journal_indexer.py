...
class JournalServiceClient:
    ...
    async def get_entry_by_id(self, entry_id: str, user_id: str) -> Optional[Dict[str, Any]]:
        ...
        logger.warning(f"Using mock JournalServiceClient for entry {entry_id}. NOT FOR PRODUCTION.")
        return {
            "id": entry_id,
            "user_id": user_id,
    ...
    async def get_all_entries_by_user(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        logger.warning(f"Using mock JournalServiceClient for user {user_id}. NOT FOR PRODUCTION.")
        return [
            {
                "id": str(UUID(int=1)),
                "user_id": user_id,
...
class JournalIndexer:
    ...
    async def index_entry(
        ...
        try:
            ...
            logger.info(f"Successfully indexed journal entry {entry_id} as point {point_id}")
            return True
    ...
    async def batch_index_entries(self, entries_data: List[Dict[str, Any]]) -> Dict[str, int]:
        ...
        for entry_data in entries_data:
            try:
                ...
                if not user_id:
                    logger.error(f"Missing user_id for entry in batch indexing: {entry_data.get('id')}")
                    failed_count += 1
                    continue
                ...
                logger.error(f"Failed to index entry {entry_data.get('id')} in batch: {e}")
                failed_count += 1
    ...
    async def reindex_user_entries(self, user_id: str, tradition: str = "canon-default") -> Dict[str, int]:
        ...
        try:
            ...
            if not entries:
                logger.warning(f"No journal entries found for user {user_id} to reindex.")
            ...
            logger.info(f"Cleared and recreated collection {collection_name} for user {user_id}")
    ...
    def _prepare_metadata(self, entry_data: Dict[str, Any], user_id: str) -> Dict[str, Any]:
...
async def index_journal_entry_by_id(entry_id: str, user_id: str, tradition: str = "canon-default") -> bool:
    ...
    try:
        ...
        entry_data = await indexer.journal_service_client.get_entry_by_id(entry_id, user_id)
...
async def delete_user_collection(user_id: str, tradition: str = "canon-default") -> bool:
    ...
    try:
        ...
        collection_name = qdrant_client._get_personal_collection_name(tradition, user_id)
