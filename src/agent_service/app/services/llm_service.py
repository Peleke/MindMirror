...
class LLMService:
    ...
    def __init__(
        ...
        if prompt_service is None:
            ...
            logger.info(
                "Created LLM service with environment-configured prompt service"
            )
        else:
    ...
    async def execute_tool(
        self, tool_name: str, arguments: Dict[str, Any], version: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
    ...
    async def execute_subtool(
        ...
        try:
            ...
            logger.error(
                f"Error executing subtool {subtool_name} of tool {tool_name}: {e}"
            )
            raise
    ...
    async def get_journal_summary(
        self, journal_entries: List[Dict[str, Any]], style: str = "concise"
    ) -> str:
        """
        ...
        try:
            if (
                self.tool_registry
                and "journal_summary_graph" in self.tool_registry.list_tool_names()
            ):
                logger.info("Using journal_summary_graph tool from registry")
                result = await self.execute_tool(
                    "journal_summary_graph",
                    {"journal_entries": journal_entries, "style": style},
                )
            ...
            logger.warning(
                f"Tool registry execution failed, falling back to direct prompt: {e}"
            )
    ...
    async def _get_journal_summary_direct(
        self, journal_entries: List[Dict[str, Any]], style: str = "concise"
    ) -> str:
        """
        ...
        try:
            ...
            rendered_prompt = self.prompt_service.render_prompt(
                "journal_summary", {"content_block": content_block, "style": style}
            )
    ...
    async def get_performance_review(
        ...
        try:
            if (
                self.tool_registry
                and "performance_review_graph" in self.tool_registry.list_tool_names()
            ):
                logger.info("Using performance_review_graph tool from registry")
                result = await self.execute_tool(
                    "performance_review_graph",
                    {"journal_entries": journal_entries, "period": period},
                )
                ...
                if result and len(result) > 0:
                    ...
                    if isinstance(result[0], dict):
                        if "review" in result[0]:
                            return self._parse_performance_review_response(
                                result[0]["review"]
                            )
                        ...
                        elif "result" in result[0] and isinstance(
    ...
    async def _get_performance_review_direct(
        self, journal_entries: List[Dict[str, Any]], period: str = "month"
    ) -> PerformanceReview:
        """
        ...
        try:
            # Render the prompt using the prompt service
            rendered_prompt = self.prompt_service.render_prompt(
                "performance_review", {"content_block": content_block, "period": period}
            )
    ...
    def _get_llm_for_task(
        self, task_name: str, metadata: Dict[str, Any]
    ) -> BaseLanguageModel:
        """
    ...
    def _parse_performance_review_response(self, response: str) -> PerformanceReview:
        ...
        try:
            ...
            if not improvement_area:
                improvement_area = (
                    "Consider adding more detailed journal entries for better insights."
                )
            if not journal_prompt:
                journal_prompt = (
                    "What would you like to focus on in your next journal entry?"
                )
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "status": (
                    "healthy"
                    if not missing_prompts and working_providers
                    else "degraded"
                ),
