...
class QdrantService:
    ...
    async def index_knowledge_documents(
        ...
        """
        try:
            self.logger.debug(f"Indexing {len(texts)} knowledge documents for tradition {tradition}")
    ...
    async def index_personal_documents(
        ...
        """
        try:
            self.logger.debug(f"Indexing {len(texts)} personal documents for user {user_id}")
            ...
            self.logger.debug(f"Successfully indexed {len(point_ids)} personal documents")
            return point_ids
