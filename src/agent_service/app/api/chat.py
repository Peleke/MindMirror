...
from agent_service.langgraph_.state import StateManager, RAGAgentState
...
async def ask_question(
    ...
    try:
        ...
        conversation_id = request.conversation_id or f"conv_{current_user.id}_{len(_conversations)}"
        ...
        # Extract response
        response = updated_state.get("last_response", "I apologize, but I couldn't generate a response.")
    ...
    try:
        ...
        for conv_id, state in _conversations.items():
            if state.get("user_id") == str(current_user.id):
                ...
                user_conversations.append(ConversationHistory(
...
async def clear_conversation(
    ...
    try:
        ...
        logger.info(f"Cleared conversation {conversation_id} for user {current_user.id}")
