        ...
        try:
            ...
            for i, embedding in enumerate(embeddings):
                if isinstance(embedding, Exception):
                    self.logger.error(f"Failed to get embedding for text {i}: {embedding}")
                    result.append(None)
                else:
                    result.append(embedding)
            
            self.logger.debug(f"Successfully processed {len([e for e in result if e is not None])} embeddings")
            return result
