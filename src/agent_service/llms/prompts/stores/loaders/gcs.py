        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if attempt == self.max_retries - 1:
                    raise PromptStorageError(
                        f"Failed to write blob {path} after {self.max_retries} attempts: {e}"
                    )
    ...
    def read_file(self, path: str) -> str:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if attempt == self.max_retries - 1:
                    raise PromptStorageError(
                        f"Failed to read blob {path} after {self.max_retries} attempts: {e}"
                    )
    ...
    def delete_file(self, path: str) -> None:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if attempt == self.max_retries - 1:
                    raise PromptStorageError(
                        f"Failed to delete blob {path} after {self.max_retries} attempts: {e}"
                    )
    ...
    def list_files(self, prefix: str = "") -> List[str]:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if attempt == self.max_retries - 1:
                    raise PromptStorageError(
                        f"Failed to list blobs after {self.max_retries} attempts: {e}"
                    )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if attempt == self.max_retries - 1:
                    raise PromptStorageError(
                        f"Failed to get GCS statistics after {self.max_retries} attempts: {e}"
                    )
