...
class YAMLPromptStore(PromptStore):
    ...
    def save_prompt(self, prompt_info: PromptInfo) -> None:
        ...
        try:
            ...
            logger.info(
                f"Saved prompt {prompt_info.name}:{prompt_info.version} to {file_path}"
            )

        except Exception as e:
            logger.error(
                f"Error saving prompt {prompt_info.name}:{prompt_info.version}: {e}"
            )
        ...
        try:
            ...
            if not file_path.exists():
                raise PromptNotFoundError(
                    f"Prompt {name}:{version} not found at {file_path}"
                )
