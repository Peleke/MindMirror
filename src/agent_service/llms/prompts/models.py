    ...
    def _validate(self):
        ...
        if self.store_type == StoreType.FIRESTORE and self.firestore_collection is None:
            raise PromptValidationError(
                "firestore_collection is required for FIRESTORE store type"
            )
...
class PromptStats:
    ...
    def average_versions_per_prompt(self) -> float:
        ...
        return (
            self.total_versions / self.total_prompts if self.total_prompts > 0 else 0.0
        )
...
class StorageConfig(BaseModel):
    ...
    local_path: Optional[str] = Field(
        None, description="Local file system path for local storage"
    )
    gcs_bucket: Optional[str] = Field(
        None, description="GCS bucket name for GCS storage"
    )
    gcs_credentials: Optional[str] = Field(
        None, description="Path to GCS credentials file"
    )
