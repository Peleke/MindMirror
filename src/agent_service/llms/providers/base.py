...
class BaseProvider(ABC):
    ...
    def _get_api_key(self, config: Dict[str, Any], env_var: str) -> str:
        ...
        if not api_key:
            raise ValueError(f"API key not found in config or environment variable {env_var}")
    ...
    def _validate_required_fields(self, config: Dict[str, Any], required_fields: List[str]) -> bool:
        ...
        for field in required_fields:
            if field not in config or not config[field]:
                logger.warning(f"Required field '{field}' missing from {self.provider_name} config")
                return False
        return True
